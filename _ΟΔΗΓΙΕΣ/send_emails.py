...
def collect_attachments(files_directory     = './STUDENTS',
    ...
    for i,folder_name in enumerate(recipient_list['FOLDER-NAME']):
        # Collect all the subfolders with the specific file
        selected_folders = [str(folder) for folder in student_folders if str(folder_name) in str(folder)]
        if len(selected_folders) > 0:
            attachment_names[i] = [att for att in listdir(selected_folders[0]) if (isfile(join(selected_folders[0], att)) and (att[0:1]!='.'))]
            attachments[i] = [selected_folders[0]+'/'+attachment_name for attachment_name in attachment_names[i]]
            success[i] = True
...
# Make folder command
def make_folders_from_xls(mainfoldername,recipient_list_dir:str):
    raw_excel = pd.read_excel(recipient_list_dir)
    recipient_list = raw_excel.to_dict()
    
    # create a new directory 
    print("---> Δημιουργία φακέλων από FOLDER-NAME")
    
    # Check if there is a folder name in the excel file
    if not 'FOLDER-NAME' in recipient_list.keys():
        print('     ΔΕΝ υπάρχει στήλη FOLDER-NAME στο αρχείο')
        print(f"---> Μη επιτυχής διαδικασία δημιουργίας φακέλων")
        return None

    # For each pdf page
    for i,foldername in enumerate(recipient_list['FOLDER-NAME']):
        # Get the flder name from the list
        foldername=recipient_list['FOLDER-NAME'][i]

        # Check if the folder name is empty
        if pd.isnull(raw_excel.iloc()[i]['FOLDER-NAME']): 
            print(f"     Foldername does not exist for {recipient_list['EMAIL'][i]}.  Skipping...")
            continue
        temp_filename = os.path.join(mainfoldername,foldername)
        
        # Create output file stream
        os.makedirs(temp_filename,exist_ok=True)

        # Get the ith row of the excel file
        row = raw_excel.iloc()[i]

        # Create the subfolders for the years
        for year in [x for x in row.keys() if 'FOLDER' in x and not '-' in x]:
            # Get a new path
            year_path = os.path.join(temp_filename,row[year])
            
            # Create the subdirectory
            os.makedirs(year_path,exist_ok=True)

            # See if there are subdirectories
            subs = [x for x in row.keys() if year+'-' in x]

            # If there are, create them
            if len(subs) > 0:
                # Concatenate the name for the directory
                sub_name = '-'.join([str(row[k]) for k in subs])

                # Create a path
                sub_path = os.path.join(year_path,sub_name)

                # Create the directory
                os.makedirs(sub_path,exist_ok=True)
        
        print("     "+temp_filename)
    print(f"---> Ολοκλήρωση για {len(raw_excel)} γραμμές από τη στήλη FOLDER-NAME")
